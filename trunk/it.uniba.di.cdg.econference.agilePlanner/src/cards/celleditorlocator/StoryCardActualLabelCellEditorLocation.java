...
public class StoryCardActualLabelCellEditorLocation implements CellEditorLocator {

    private StoryCardModel storyModel;

    private StoryCardEditPart storyEditPart;

    public StoryCardActualLabelCellEditorLocation(StoryCardModel model, StoryCardEditPart editPart) {
    ...
    public void relocate(CellEditor celleditor) {
        ...
        celleditor.addListener(new StoryCardLiveTextEditorListener(storyModel, celleditor, CardConstants.ActualLabel));
        Text text = (Text) celleditor.getControl();
        text.setFont(new Font(null,storyEditPart.getStoryCardFont(),(storyEditPart.getStoryCardFontSize()*((int)ratio)),0));
        int xRelative = (int)((storyEditPart.getActualLabel().getLocation().x - storyEditPart.getFigure().getBounds().x)*ratio);
        int yRelative = (int)((storyEditPart.getActualLabel().getLocation().y - storyEditPart.getFigure().getBounds().y)*ratio);
