...
public class IterationCardNameLabelCellEditorLocation implements CellEditorLocator {

    private IterationCardFigure iterationFigure;
    private IterationCardEditPart editpart;

    public IterationCardNameLabelCellEditorLocation(IterationCardFigure figure,IterationCardEditPart editpart) {
    ...
    public void relocate(CellEditor celleditor) {
        ...
        /** Divide width by 2 because this is the first label on this line */
        rect.width = rect.width
                + trim.width
                - ((rect.width / 2)
                + ((System.getProperty("os.name").equalsIgnoreCase("Mac OS X"))
                		? CardConstants.MacIterationFigureNamePoint
                		: CardConstants.WindowsIterationFigureNamePoint) +7);
        ...
        text.setBounds(rect.x
                + ((System.getProperty("os.name").equalsIgnoreCase("Mac OS X")) ? CardConstants.MacIterationFigureNamePoint
                        : (int)(CardConstants.WindowsIterationFigureNamePoint*ratio)), (int)((rect.y + 4)*ratio),(int)( rect.width*ratio), (int)(rect.height*ratio));
    }
