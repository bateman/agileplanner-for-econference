...
public class StoryCardMostLikeylCellEditorLocation implements CellEditorLocator {

    private StoryCardModel storyModel;

    private StoryCardEditPart storyEditPart;

    public StoryCardMostLikeylCellEditorLocation(StoryCardModel model, StoryCardEditPart editPart) {
    ...
    public void relocate(CellEditor celleditor) {
        ...
        celleditor.addListener(new StoryCardLiveTextEditorListener(storyModel, celleditor, CardConstants.MostLikelyLabel));
        Text text = (Text) celleditor.getControl();
        text.setFont(new Font(null,storyEditPart.getStoryCardFont(),(storyEditPart.getStoryCardFontSize()*((int)ratio)),0));
        int xRelative = (int)((storyEditPart.getMostLikelyLabel().getLocation().x - storyEditPart.getFigure().getBounds().x)*ratio);
        int yRelative = (int)((storyEditPart.getMostLikelyLabel().getLocation().y - storyEditPart.getFigure().getBounds().y)*ratio);
